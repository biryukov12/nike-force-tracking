...
from parse import parsing_data
...
async def get_data(message: types.Message):
    ...
    if parsing_data is None:
        ...
        for i in range(len(products)):
            ...
            product_url = "https://nike.com/ru" + products[i]["url"].strip(
                "{countryLang}"
            )
...
async def start(message: types.Message):
    ...
    await message.answer(
        "Select: once parsing or schedule parsing", reply_markup=keyboard
    )
...
async def schedule_start(message: types.Message):
    try:
        ...
        scheduler.add_job(
            get_data, trigger="interval", hours=1, args=(message,), id="1_hour"
        )
    except Exception as e:
        log.error(e)
